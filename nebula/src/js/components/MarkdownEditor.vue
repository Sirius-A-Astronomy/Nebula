...
const options = computed(() => {
  if (props.preset && presets[props.preset!]) {
    return Object.entries(presets[props.preset!].options).reduce(
      (acc, [key, value]) => {
        if (props.options?.[key as keyof typeof props.options] === undefined) {
          acc[key] = value;
        }
        return acc;
      },
...
const customText = computed(() => {
  if (props.preset) {
    return Object.entries(presets[props.preset!].customText).reduce(
      (acc, [key, value]) => {
        if (
          props.customText?.[key as keyof typeof props.options] === undefined
        ) {
          acc[key] = value;
        }
        return acc;
  ...
  <div class="markdown-editor" :class="`preset-${preset}`">
    ...
    >
      ...
      >
        ...
        />
        <div class="syntax-highlighted" v-html="highlightedContent"></div>
      </div>
      <Markdown
        class="vp-markdown-preview"
        v-show="selectedTab === 'rendered' || selectedTab === 'side-by-side'"
        :options="{
