...
def create_question():
    ...
    if subject_tags_req is not None:
        ...
        for subject_tag in subject_tags_req:
            ...
            existing_subject_tag = SubjectTag.query.filter_by(name=subject_tag.get("name").lower().strip()).one_or_none()
...
def update_question(uuid):
    ...
    if subject_tags_req is not None:
        ...
        for subject_tag in subject_tags_req:
            ...
            if (db.session.execute(db.select(Question).where(Question.uuid == question.uuid).where(Question.subject_tags.any(SubjectTag.name == subject_tag.get("name").lower().strip()))).scalar() is None):
                # if it does check if the subject tag exists
                existing_subject_tag = SubjectTag.query.filter_by(name=subject_tag.get("name").lower().strip()).one_or_none()
                if existing_subject_tag is None:
                    # create it if not
        ...
        for subject_tag in question.subject_tags:
            ...
            if (db.session.execute(db.select(SubjectTag).where(SubjectTag.name.in_([subject_tag.get("name").lower().strip() for subject_tag in subject_tags_req]))).scalar() is None):
