...
class QuestionEditForm(FlaskForm):
    ...
    title = StringField("Edit Title", validators=[DataRequired("Please enter a title")])
    content = TextAreaField(
        "Edit Question Content",
        validators=[DataRequired("Please enter a question content")],
    )
    ...
    difficulty = SelectField(
        ...
        coerce=int,
        validators=[DataRequired("Please select a difficulty")],
    )
...
class AnswerForm(FlaskForm):
    ...
    content = TextAreaField("Answer content", validators=[DataRequired()])
    sources = TextAreaField("Add sources", validators=[Optional()])
...
def question(
    course_code, question_uuid, course_level_code, new_comment_uuid=None, edit=None
):
    ...
    if comment_form.comment_submit.data == True and comment_form.validate():
        ...
        return redirect(
            url_for(
                ...
                new_comment_uuid=str(comment.uuid),
            )
        )

    if (
        question_edit_form.question_edit_submit.data == True
        and question_edit_form.validate()
    ):
        if current_user.is_anonymous or (
            current_user != question.user and current_user.access_level <= 2
        ):
        ...
        question.difficulty = (
            "Easy"
            if question_edit_form.difficulty.data == 1
            else "Medium"
            if question_edit_form.difficulty.data == 2
            else "Hard"
        )
        db.session.commit()
    ...
    if add_answer_form.add_answer_submit.data == True and add_answer_form.validate():
        ...
        sources = [
            ...
            .geturl()
            .replace("///", "//")
            for source in add_answer_form.sources.data.split(";")
            if source
        ]
        answer = Answer(content=content, sources=sources, user=user, question=question)
